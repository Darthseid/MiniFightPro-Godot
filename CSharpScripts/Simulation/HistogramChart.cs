...
public partial class HistogramChart : Control
{
    ...
    {
        DrawRect(new Rect2(Vector2.Zero, Size), new Color(0.10f, 0.10f, 0.12f));

        if (_series.Count == 0 || _series.All(s => s.Values.Count == 0))
        {
            DrawString(ThemeDB.FallbackFont, new Vector2(18, 36), "Run a simulation to render histograms.", HorizontalAlignment.Left, -1f, 16, Colors.White);
            return;
        }
        ...
        var right = 20f;
        var bottom = 74f;
        var plot = new Rect2(left, top, Math.Max(120f, Size.X - left - right), Math.Max(120f, Size.Y - top - bottom));

        DrawString(ThemeDB.FallbackFont, new Vector2(12, 28), _title, HorizontalAlignment.Left, -1f, 18, Colors.White);

        var min = _series.SelectMany(s => s.Values).DefaultIfEmpty(0d).Min();
        var max = _series.SelectMany(s => s.Values).DefaultIfEmpty(1d).Max();
        ...
        foreach (var series in _series)
        {
            ...
            peak = Math.Max(peak, bins.Max());
            histograms.Add(bins);
        }

        DrawLine(plot.Position + new Vector2(0, plot.Size.Y), plot.Position + plot.Size, Colors.White);
        DrawLine(plot.Position, plot.Position + new Vector2(0, plot.Size.Y), Colors.White);

        var groupWidth = plot.Size.X / binCount;
        var barWidth = groupWidth / Math.Max(1, _series.Count + 0.5f);
        ...
        for (var bin = 0; bin < binCount; bin++)
        {
            for (var s = 0; s < _series.Count; s++)
            {
                ...
                DrawRect(new Rect2(x, y, Math.Max(2f, barWidth - 4f), barHeight), _series[s].Color);
            }
        }

        DrawString(ThemeDB.FallbackFont, new Vector2(plot.Position.X, plot.Position.Y + plot.Size.Y + 22), min.ToString("0.##", CultureInfo.InvariantCulture), HorizontalAlignment.Left, -1f, 12, Colors.White);
        DrawString(ThemeDB.FallbackFont, new Vector2(plot.Position.X + plot.Size.X - 40, plot.Position.Y + plot.Size.Y + 22), max.ToString("0.##", CultureInfo.InvariantCulture), HorizontalAlignment.Left, -1f, 12, Colors.White);
        DrawString(ThemeDB.FallbackFont, new Vector2(plot.Position.X + (plot.Size.X / 2f) - 40f, Size.Y - 18), _xAxisLabel, HorizontalAlignment.Left, -1f, 12, Colors.White);
        ...
        for (var i = 0; i < _series.Count; i++)
        {
            ...
            DrawString(ThemeDB.FallbackFont, new Vector2(legendX + 20 + (i * 170), legendY + 12), $"{_series[i].Label} (avg {mean:0.##})", HorizontalAlignment.Left, -1f, 12, Colors.White);
        }
    }
