...
public sealed class DuelSimulator
{
    ...
    public DuelBatchResult RunBatch(DuelConfig config, Squad a, Squad b, int trials, int? seed)
    {
        ...
        for (var i = 0; i < safeTrials; i++)
        {
            ...
            {
                aWins++;
                aWinnerHpSum += single.WinnerRemainingHealth;
            }
            else
            {
                bWins++;
                bWinnerHpSum += single.WinnerRemainingHealth;
            }
        ...
        return new DuelBatchResult
        {
            ...
            AverageHpRemainingWhenSquadAWins = aWins > 0 ? (double)aWinnerHpSum / aWins : 0,
            AverageHpRemainingWhenSquadBWins = bWins > 0 ? (double)bWinnerHpSum / bWins : 0,
            AverageSquadADamagePerTrial = (double)totalADamage / safeTrials,
            AverageSquadBDamagePerTrial = (double)totalBDamage / safeTrials,
            FirstAttackerWinPercent = (double)firstAttackerWins * 100d / safeTrials,
    ...
    private void ResolveAttackStep(DuelConfig config, SimSquadState attacker, SimSquadState defender, bool isMelee, bool attackerIsA, ref bool roundDidDamage, bool allowRetaliation = true)
    {
        ...
        foreach (var batch in weaponBatches)
        {
            ...
            hits += hardHits;
            var (injuries, devastating) = CombatRolls.WoundSequence(hits, weapon.Strength, defender.WorkingSquad.Hardness, modifiers.WoundMod, modifiers.WoundReroll, weapon.Special, modifiers.AntiThreshold);
            ...
            ApplyUnsavedInjuries(attacker, defender, weapon, unsaved, config.RangeInches, attackerIsA, ref roundDidDamage);
            ...
            if (allowRetaliation && isMelee && defender.HasFightOnDeath && defenderAliveAfter < defenderAliveBefore && !defender.IsDestroyed)
                ResolveAttackStep(config, defender, attacker, true, !attackerIsA, ref roundDidDamage, allowRetaliation: false);
    ...
    private void ApplyUnsavedInjuries(SimSquadState attacker, SimSquadState defender, Weapon weapon, int unsaved, float range, bool attackerIsA, ref bool roundDidDamage)
    {
        for (var i = 0; i < unsaved; i++)
        {
            ...
            var target = defender.WorkingSquad.Composition.Where(m => m.Health > 0).OrderBy(m => m.Health).FirstOrDefault();
            if (target == null)
                break;
            ...
            for (var p = 0; p < finalDamage && target.Health > 0; p++)
            {
                ...
                if (!negated)
                {
                    target.Health -= 1;
                    roundDidDamage = true;
                    if (attackerIsA) _trialSquadADamage++; else _trialSquadBDamage++;
                }
            }
        }
        ...
        if (deadBefore > 0)
            HandleExplosions(defender, attacker, deadBefore, range, !attackerIsA, ref roundDidDamage);
    }

    private void HandleExplosions(SimSquadState exploded, SimSquadState other, int deadCount, float range, bool explodedIsA, ref bool roundDidDamage)
    {
        ...
        for (var i = 0; i < deadCount; i++)
            if (DiceHelpers.SimpleRoll(6) == 1) triggers++;

        if (triggers <= 0)
            return;

        var blast = explodeDamage * triggers;
        var selfKills = CombatRolls.AllocatePure(blast, exploded.WorkingSquad);
        exploded.RemoveDeadAndHandleSecondLife();
        if (selfKills > 0)
        {
            roundDidDamage = true;
            if (explodedIsA) _trialSquadADamage += blast; else _trialSquadBDamage += blast;
        }

        if (range <= 6f)
        {
            var otherKills = CombatRolls.AllocatePure(blast, other.WorkingSquad);
    ...
    private static void ResolvePerilous(SimSquadState attacker, Weapon weapon, ref bool roundDidDamage)
    {
        ...
        foreach (var model in attacker.WorkingSquad.Composition.Where(m => m.Health > 0))
        {
            ...
            if (!hasWeapon || DiceHelpers.SimpleRoll(6) != 1)
                continue;

            if (attacker.IsInfantry) model.Health = 0;
            else model.Health = Math.Max(0, model.Health - 3);
            roundDidDamage = true;
        }
