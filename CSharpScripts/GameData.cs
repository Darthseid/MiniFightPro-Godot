...
public partial class GameData : Node
{
    ...
    private static bool SyncPlayerOrders(Player player)
    {
        ...
        foreach (var order in currentOrders)
        {
            ...
            if (!string.IsNullOrWhiteSpace(order.OrderId) && ordersById.TryGetValue(order.OrderId, out var latestById))
            {
                updatedOrders.Add(latestById);    // First try to match by id.
                continue;
            }
